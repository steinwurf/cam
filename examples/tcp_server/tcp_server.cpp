...
public:
    ...
    connection(ba::ip::tcp::socket client, camera_type camera)
...
private:
    ...
    void do_capture(boost::system::error_code error)
    {
        if (error)
        {
            m_client.reset();
    ...
    void do_accept()
    {
        ...
        m_camera = std::unique_ptr<camera_type>(new camera_type);
...
private:
    ...
    std::unique_ptr<camera_type> m_camera;
    ...
    // c4m::linux::camera2<c4m::default_features> m_camera;
