...
namespace linux
{
    ...
    ///    auto buffers = c4m::linux::stop_streaming(fd, &error);
    ///
    ///    if (error)
    ///    {
    ...
    ///
    inline void stop_streaming(const scoped_file_descriptor& fd,
                               std::error_code& error)
    {
        assert(fd);

        uint32_t type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
        retry_ioctl(fd, VIDIOC_STREAMOFF, &type, error);
    }

    /// Calls stop_streaming(...) with an error_code and throws an
